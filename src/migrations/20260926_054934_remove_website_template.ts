...
/**
 * Drops the website-template collections, globals and Pages blocks removed
 * in H1. The column drops only remove the `posts_id` / `categories_id` /
 * ... references, not the rows holding them, so first delete every row that
 * points at a removed document: page and page-version relationships, locks
 * on removed documents and globals, and pending scheduled post publishes
 * (which would otherwise fail on every cron run). Template data is gone for
 * good; take a backup before deploying.
 */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  // Drop the foreign keys into the removed tables first: `DROP TABLE ...
  // CASCADE` removes them too, and the generated `DROP CONSTRAINT` after it
  // then fails on a constraint that no longer exists.
  await db.execute(sql`
...
// Recreates the template schema empty; the dropped data isn't restored.
