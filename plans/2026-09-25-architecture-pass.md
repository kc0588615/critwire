...
- [x] **Step 12: Atomic report promotion (F5)**
...
- [x] **Step 13: Contact routing helper and shared public-form guard (F3 part 2, F12)**
  - **Scenarios:** remove the `test.fail` from S5.2. S5.1, S5.4 and S5.5 guard the refactor.
  - **Files:**
    - new `src/lib/game-portal/formRoutes.ts` (was `contactRoute.ts`; see Decisions): `getContactRoute(slug)`, returning the union without secrets, and `getReportRoute(reportForm)`
...
- [x] **Step 14: Contact jobs deliver or throw, and Discord destinations are restricted (F13, F21)**
...
- [x] **Step 16: Kanban shows only moves the server accepted (F6)**
...
- [x] **Step 17: Remaining DRY helpers (F14)**
  - **Files:**
...
- [x] **Step 17a: Remove the website-template code (H1, part 1)**
...
- [x] **Step 17b: Migration that drops the template tables (H1, part 2)**
...
- [x] **Step 17c: Backfill media-folder tenants (H2)**
...
- [x] **Step 20: Testing rules and deploy docs**
...
- [ ] **Step 21: Final verification, artifact and Summary**
  - **Do:**
    ...
    3. Write the Summary:
       ...
       - the bugs found and fixed: F1–F13, F21, F22, F24, F25 and F26 (Step 20), one line each. H1 superseded some of them by removing the surface they protected: F2's post, category, header, footer, redirect, form and search locks, F16, and F10's `ArchiveBlock` fix. Say so in their lines.
...
- **Step 12: the promotion hook has no Sentry wrapper of its own.** It runs inside the report's write, so a failure fails and rolls back the whole request, which reports it; the old wrapper existed only because the link was written detached. Same as `adjustUpvoteCount`.
- **Step 12: the merged report is cast to `IssueReport`.** `beforeChange` runs after field validation, so `title`, `description` and `category` are present; only project and tenant are checked, and a missing one throws `ValidationError` on `status`.
- **Step 12: S5.3's guard test no longer polls for the link.** It reads `issue` from the PATCH response, as Step 11's deviation note allowed.
- **Step 13: the report page and route share one routing decision, `getReportRoute(reportForm)`, instead of the route checking `provider !== 'native'`.** The page falls back to the native form when a Tally provider has no usable URL or an external provider has no link; a bare provider check would then show a form whose every submission answered 400. Both now read the same union (`tally` | `external` | `native`), and the route rejects anything but `native`. It sits with `getContactRoute` in `src/lib/game-portal/formRoutes.ts` (the plan named it `contactRoute.ts`), which shares the Tally display default. It's pure: `reportForm` is publicly readable, so it works on the visitor's `getGameProject` view.
- **Step 13: `guardPublicForm({ req, schema, rateLimit: { key, limit, scope, windowSeconds } })` reads the body and the client IP itself**, rather than taking `body` and `ip` as the architecture sketched: both routes did exactly that first, and the rate-limit identifier is always `<ip>:<scope>`. The Turnstile token is read with `normalizeTurnstileToken` apart from the schema, so neither form schema carries `turnstileToken` any more (Zod strips it).
...
- **Step 16: a move is planned by a pure `planMove(columns, …)`, and rollback restores only the two columns it touched.** The handler sets the planned board, then sends the PATCH; on failure a pure updater puts back the snapshot's `from` and `to` columns (docs, page and count together) and `toast.error` fires. Restoring whole columns keeps each column's page and docs consistent even if a load-more landed meanwhile; that column's "Load more" simply fetches the page again. The only other `setColumns((prev) => …)` left is load-more's append, which is pure.
- **Step 16: serialized moves use a ref, not disabled sensors.** A drag that starts while a PATCH is in flight is ignored (`sourceCol` stays null, so drop is a no-op). The unused `overColumnRef` and the `isDragging` ref went; `sourceCol` being set means a drag is active.
- **Step 16 (test fix): S6.2's not-found redirect asserts the path and `notFound` param, not the exact URL.** Payload's list view sometimes appends its default `depth`, `limit` and `sort` params after the redirect, which the anchored regex rejected (failed once in this step's first run, unrelated to the kanban change).
- **Planner H1 decisions (Step 17a): taken.** Code first, migration in Step 17b, and no `migrate:create` in between. The Archive and Form blocks left Pages; the header renders only the logo link and the footer the logo and theme selector, with no nav replacing the globals'; `link.reference` stays polymorphic with `relationTo: ['pages']`; the eight packages whose last importer went were removed; S1 lost one test net (S1.7's template checks and F16 became "the website-template surface is gone [H1]", F2 now guards Pages, S1.9 lost its Archive test).
- **Step 17a: an unresolvable rich-text internal link renders `'#'`** instead of throwing (planner decision). `internalDocToHref` reads `linkNode.fields.doc?.value` and returns `/${slug}` only for a populated page. With no editor left on `BlocksFeature`, `RichText` has no block converters and uses Lexical's default node types.
- **Step 17a: `src/Header/Component.tsx` would have been an empty wrapper**, so the client component took its place (`git mv`), still exporting `Header` for `layout.tsx`.
- **Step 17a: stale `.next/dev/types` was cleared too**, not only `.next/types`. A dev run from before the mission left a `validator.ts` there that imports the deleted post and search routes, and tsc includes it.
- **Step 17a: the pages sitemap has no default entries now.** With `/search` and `/posts` gone, the list is just the published pages (and empty when build-time static generation is skipped).
- **Planner H1 decision (Step 17b): taken.** The drop migration deletes dangling references before the generated DDL: page and page-version relationships to posts and categories, locks on removed documents and on the header and footer globals, and pending `schedulePublish` jobs for posts. `payload_preferences` rows for removed collections stay. `down` stays as generated and recreates the template schema empty.
...
- 2026-09-26 05:32 UTC: Step 16 done: kanban moves are planned outside state updaters, rolled back with an error toast when the PATCH fails, and serialized; load-more failures toast; `list.tsx` no longer turns query errors into empty columns. S6.5 un-annotated and passes. First E2E run failed S6.2 on an over-strict redirect URL assertion (Payload appended its default list params); assertion fixed. tsc 0; lint 0 errors, 27 warnings (unchanged); E2E exit 0, 72 passed, 0 expected-fail, 0 flaky, 219 s wall; `dev` rows 0 in both DBs.
- 2026-09-26 05:40 UTC: Step 17 done: F14 finished. The local relation-ID helpers left in `validateReportStatus`, `validateUniqueSlugPerProject`, the GamePages one-page-per-kind hook, `site-generator/service.ts` and `context.ts`, `tenantAccess`, `tenantRoles`, `/next/site-preview`, the Critter Connect seed and the flagship defaults now use `extractID` with explicit null guards; `validateReportStatus` needs no helper at all. The Issues and IssueReports same-project `filterOptions` are one `src/fields/sameGameProjectFilter.ts`. Left for Step 17a: `Posts/hooks/populateAuthors.ts`, deleted there. No behavior change. tsc 0; lint 0 errors, 27 warnings (unchanged); `site-generator.int` 6/6 and `template-revalidation.int` 3/3; E2E exit 0, 72 passed, 0 expected-fail, 0 flaky, 219 s wall; `dev` rows 0 in both DBs.
